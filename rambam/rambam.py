...
import rambam.helpers.keywordmessageparser
import rambam.sources.biblegateway
import rambam.sources.filmot
import rambam.sources.sefaria

SefariaAPI = rambam.sources.sefaria.SefariaAPI()
YouTubeSearch = rambam.sources.filmot.YouTubeTranscriptSearch()
BibleGateway = rambam.sources.biblegateway.BibleGateway()
KeywordReferenceSearch = rambam.helpers.keywordmessageparser.KeywordMessageParser()
...
async def youtube_transcript_search(ctx: discord.ApplicationContext, search: str):
    ...
    youtube_search = rambam.sources.filmot.YouTubeTranscriptSearch()
    results = youtube_search.search_transcripts(query=search)
    embeds = youtube_search.create_embeds(results=results)
